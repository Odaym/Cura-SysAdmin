...
public class DynamicGridView extends GridView {
	...
	private OnItemLongClickListener mLocalLongClickListener = new OnItemLongClickListener() {
		public boolean onItemLongClick(AdapterView<?> arg0, View arg1, int pos,
			...
			if (mSelectedItemBitmapCreationListener != null) {
				mSelectedItemBitmapCreationListener.OnPreSelectedItemBitmapCreation(
						selectedView, position, mMobileItemId);
			}
			...
			if (mSelectedItemBitmapCreationListener != null) {
				mSelectedItemBitmapCreationListener.OnPostSelectedItemBitmapCreation(
						selectedView, position, mMobileItemId);
			}
	...
	/**
	 * Creates the hover cell with the appropriate bitmap and of appropriate size.
	 * The hover cell's BitmapDrawable is drawn on top of the bitmap every single
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mCellIsMobile) {
				mHoverCellCurrentBounds.offsetTo(mHoverCellOriginalBounds.left + deltaX
						+ mTotalOffsetX, mHoverCellOriginalBounds.top + deltaY
						+ mTotalOffsetY);
		...
		case MotionEvent.ACTION_UP:
			...
			if (undoSupportEnabled) {
				if (currentModification != null
						&& !currentModification.getTransitions().isEmpty()) {
		...
		case MotionEvent.ACTION_POINTER_UP:
			/*
			 * If a multitouch event took place and the original touch dictating the
			 * movement of the hover cell has ended, then the dragging event ends and
			 * the hover cell is animated to its corresponding position in the
			 * listview.
			 */
	...
	private void touchEventsEnded() {
		...
		if (mCellIsMobile || mIsWaitingForScrollFinish) {
			...
			// If the autoscroller has not completed scrolling, we need to wait for it
			// to
		...
		TypeEvaluator<Rect> sBoundEvaluator = new TypeEvaluator<Rect>() {
			public Rect evaluate(float fraction, Rect startValue, Rect endValue) {
				return new Rect(interpolate(startValue.left, endValue.left, fraction),
						interpolate(startValue.top, endValue.top, fraction), interpolate(
								startValue.right, endValue.right, fraction), interpolate(
	...
	/**
	 * + * The GridView from Android L requires some different setVisibility()
	 * logic + * when switching cells. Unfortunately, both 4.4W and the
	 * pre-release L + * report 20 for the SDK_INT, but we want to return true for
	 * 4.4W and false + * for Android L. So, we check the release name for "L" if
	 * we see SDK 20. + * Hopefully, Android L will actually be SDK 21 or later
	 * when it ships.
	 *
	 * @return
	 */
	public static boolean isPreL() {
		...
		return (Build.VERSION.SDK_INT < KITKAT_WATCH)
				|| ((Build.VERSION.SDK_INT == KITKAT_WATCH) && !"L"
						.equals(Build.VERSION.RELEASE));
	}
	...
	private void handleCellSwitch() {
		...
		final int deltaYTotal = mHoverCellOriginalBounds.centerY() + mTotalOffsetY
				+ deltaY;
		final int deltaXTotal = mHoverCellOriginalBounds.centerX() + mTotalOffsetX
				+ deltaX;
		...
		for (Long id : idList) {
			...
			if (view != null) {
				...
				if ((aboveRight(targetColumnRowPair, mobileColumnRowPair)
						...
						&& deltaYTotal > view.getTop() && deltaXTotal < view.getRight() || above(
					...
					float xDiff = Math.abs(com.cura.gridview.DynamicGridUtils.getViewX(view)
							- com.cura.gridview.DynamicGridUtils.getViewX(mMobileView));
					float yDiff = Math.abs(com.cura.gridview.DynamicGridUtils.getViewY(view)
							- com.cura.gridview.DynamicGridUtils.getViewY(mMobileView));
	...
	private class KitKatSwitchCellAnimator implements SwitchCellAnimator {
		...
		public void animateSwitchCell(final int originalPosition,
			...
			getViewTreeObserver().addOnPreDrawListener(
					new AnimateSwitchViewOnPreDrawListener(mMobileView, originalPosition,
							targetPosition));
			mMobileView = getViewForId(mMobileItemId);
		}
			...
			for (int pos = Math.min(oldPosition, newPosition); pos < Math.max(
				...
				if ((pos + 1) % getColumnCount() == 0) {
					resultList.add(createTranslationAnimations(view, -view.getWidth()
							* (getColumnCount() - 1), 0, view.getHeight(), 0));
				} else {
	...
	private AnimatorSet createTranslationAnimations(View view, float startX,
		...
		ObjectAnimator animX = ObjectAnimator.ofFloat(view, "translationX", startX,
				endX);
		ObjectAnimator animY = ObjectAnimator.ofFloat(view, "translationY", startY,
				endY);
	 ...
	 * swapping when the cell is either at the top or bottom edge of the gridview.
	 * If the hover cell is at either edge of the gridview, the gridview will
	 * begin scrolling. As scrolling takes place, the gridview continuously checks
	 * if new cells became visible and determines whether they are potential
	 * candidates for a cell swap.
	 */
	private OnScrollListener mScrollListener = new OnScrollListener() {
			...
			for (int i = 0; i < visibleItemCount; i++) {
				...
				if (child != null) {
					if (mMobileItemId != INVALID_ID
							&& Boolean.TRUE != child.getTag(R.id.dynamic_grid_wobble_tag)) {
						if (i % 2 == 0)
							animateWobble(child);
						else
		 ...
		 * continued. Secondly, if the hover cell has already been released, this
		 * invokes the animation for the hover cell to return to its correct
		 * position after the gridview has entered an idle scroll state.
		 */
		...
		/**
		 * Determines if the gridview scrolled down enough to reveal a new cell at
		 * the bottom of the list. If so, then the appropriate parameters are
		 * updated.
		 */
